  ...
  <div id="posts">
    <p class="border p-3 my-4" v-for="post in posts" :key="post.id">
      ...
      <router-link :to="{ name: 'update', params: {postId: post.id} }">
        <button class="p-1 mx-3 float-right btn btn-light" type="button">Editar News</button>
      </router-link>
      <button
    ...
    <div>
      <button
        ...
        @click="navigate(next)"
      >Siguiente</button>
      <button class="m-3 btn btn-primary" type="button" v-if="prev" @click="navigate(prev)">Anterior</button>
    </div>
  </div>
</template>
